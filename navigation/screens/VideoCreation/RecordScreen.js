...
export default function RecordScreen({ navigation }) {
  ...
  async function handleGetVideo() {
    ...
    if (!result.canceled) {
      ...
      navigation.navigate("Playback", { videoUri: result.assets[0].uri });
    }
  }
  ...
  const takeVideo = async () => {
    if (camera) {
      ...
      navigation.navigate("Playback", { videoUri: data.uri });
    }
  };
    ...
    return (
      <SafeAreaView style={Styles.appContainer}>
        ...
        <View style={Styles.buttonContainer}>
          <TouchableOpacity
            style={[Styles.smallButton, isRecording ? {backgroundColor: "transparent"} : {}]}
            onPress={handleGetVideo}
            disabled={isRecording}
            underlayColor="#fff"
          >
            <Ionicons style={[Styles.smallIcon, isRecording ? {color: "transparent"} : {}]} 
            name={"images-outline"} />
          </TouchableOpacity>
          ...
          >
            <Ionicons
              style={[Styles.smallIcon, isRecording ? {color: "transparent"} : {}]}
