...
export default function HomeScreen({ navigation }) {
	...
	//const height = 789.4761904761905; // Alyssa emulator height
	...
	const fetchVideos = async () => {
		...
		try {
			...
			const res = await VideoDataService.getAll(data_chunck);
	...
	const onViewableItemsChanged = useRef(({ changed }) => {
		changed.forEach(element => {
			const cell = cellRef.current[element.key];
			if (cell) {
    ...
    const fetchHasReportedVideos = async () => {
        try {
				...
				>
					...
					>
						<PostSingle
							...
							item={item}
							ref={PostSingleRef =>
								(cellRef.current[item.id] = PostSingleRef)
							}
						/>
					</TouchableOpacity>
				</View>
